...
agent_stonks/
  ...
  apple_trader.py — Apple Trader: rule-based (no LLM) loop that buys a change into positive momentum
                  the model expects to persist, and exits on a trailing stop below the high since
                  entry — or, when armed, on the model forecasting that regime flipping negative
