...
score *every* closed bar of the session instead of sleeping on armed
conditions, so there is nothing to fast-forward past -- see ``_run_rule_day``.
"""
...
from agent_stonks import clock, market_hours
...
class SimulationEngine:
    ...
    def rule_based(self) -> bool:
        """Whether this run is one of the non-LLM rule agent loops."""
        return is_rule_based(self.config.personality)
    ...
    def _build_rule_trader(self) -> Any:
    ...
    def _run_rule_day(self, day: date, trader: Any) -> None:
        ...
        for t in self.market.step_times(day):
            ...
            if not market_hours.is_market_open(t):
                continue
            self.cycles_run += 1
            outcome = trader.run_cycle(self.app, self.tracker)
